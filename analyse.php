...
// Requête pour sélectionner les données de la table 'analyse'
$query = "SELECT id_analyse, nom_analyse FROM analyse";
...
<button id="retourButton">Retour</button> <!-- Bouton de retour ajouté ici -->
  ...
  <h1>Choisir les Analyses</h1>

  <center>
    <div class="searchResults">
      <input type="text" id="searchInput" placeholder="Rechercher des Analyses">
      <ul id="searchResultsList">
        <?php foreach ($data as $row): ?>
          <li class="searchResultItem" data-id="<?php echo $row['id_analyse']; ?>"><?php echo $row['nom_analyse']; ?></li>
        <?php endforeach; ?>
      </ul>
    </div>

    <div class="selectedMedicaments">
      <h2>Analyses sélectionnés</h2>
  ...
  <script>
    $(document).ready(function() {
      ...
      $('#validerButton').on('click', function() {
        ...
        $.post('trettt2.php', { selectedMedicaments: selectedMedicaments , patient_id:patient_id ,id_medecin:id_medecin , date:date })
        .done(function(data) {
